        ...
        public void ProcessInput(ConsoleKeyInfo key_info)
        {
            ...
            if (user_choice > 0 && user_choice <= 3)
            {
                switch(user_choice)
                {
                    case 1:
                        {
                            _stateMachine.ChangeState(new ShopBuyingState(_stateMachine, this, _shopGameObject,_player));
                            break;
                        }
                    case 2:
                        {
                            _stateMachine.ChangeState(new ShopSellingState(_stateMachine, this, _shopGameObject, _player));
                            break;
                        }
                    case 3:
                        {
